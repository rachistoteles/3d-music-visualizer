...
function init(
  ...
  camera.position.z =25;
    ...
    //camera.lookAt(0, 0, 0); // Make sure the camera is looking at the origin
    ...
    if (features === "Moving Rotation") {
      ...
      if (inOrderObjects) {
        for (let i = 1; i <= numModels; i++) {
          ...
          switch (modelName) {
            case "sasuke":
              ...
              position =
              new THREE.Vector3(
                (-distanceBetween * 0.5 * i) / numModels,
                (distanceBetween * 0.5* i) / numModels,
               0
              );
              break;
              case "infernape":
                ...
                } else { // If index is odd, invert the position
                    position = new THREE.Vector3(
                        -(distanceBetween * i) / 10,
                        0,
                        0,
          ...
          loadModel(
            ...
            modelPath,
            {
              ...
              scale: new THREE.Vector3(size*1.2, size*1.2, size*1.2),
              speed: speed,
              size: size,
            },
            (model, name) => addGltfModel(model, speed, name, size)
        ...
        for (let i = 1; i <= numModels; i++) {
          ...
          let size = 1;
          let position;
          const sizes = [1.4, 1.3, 0.15, 0.0022];
    ...
    createCameraGUI(camera, controls, 35);
    ...
    const render = () => {
      ...
      gltfModels.forEach(({ model, speed, scale }) => {
        ...
        function moveCameraToQuadrant(quadrant: string) {
          switch (quadrant) {
              case 'front':
              ...
              case 'right':
                  ...
                  break;
              default:
                  console.warn('Invalid quadrant specified');
          }
          camera.lookAt(new THREE.Vector3(0, 0, 0)); // Adjust the lookAt position if needed
      }
        if (includeBackgroundColor) {
          ...
          if (averageFreq < 50) {
            ...
            camera.position.y += camera.position.y * deltaPosition;
            moveCameraToQuadrant('back');
            ...
            camera.rotation.x += rotationSpeed;
            camera.rotation.z += rotationSpeed;
            moveCameraToQuadrant('left');
          } else {
            scene.background = new THREE.Color("black");
            camera.rotation.y += rotationSpeed;
            camera.rotation.z += rotationSpeed;
            moveCameraToQuadrant('right');
          }
