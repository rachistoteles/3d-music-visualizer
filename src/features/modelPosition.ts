...
export function updateModelPosition(model: THREE.Group, averageFreq: number, deltaPosition: number) {
    if (averageFreq < 50) {
        model.scale.y += 2.5 * deltaPosition;
        model.scale.z += 2.5 * deltaPosition;
        model.scale.x -= 2.5 * deltaPosition;
    } else if (averageFreq >= 50 && averageFreq <= 70) {
        model.scale.y += 1.6 * deltaPosition;
        model.scale.z += 1.6 * deltaPosition;
        model.scale.x -= 1.6 * deltaPosition;
        ...
        model.scale.x += 2.5 * deltaPosition;
        model.scale.y -= 3 * deltaPosition;
        model.scale.z -= 3 * deltaPosition;
    }
